...
class InventoryAlerter:
    ...
    def generate_email_html(self, alerts: Dict[str, pd.DataFrame], 
    ...
    def send_email_alert(self, alerts: Dict[str, pd.DataFrame], 
        ...
        for category, df in alerts.items():
            if not df.empty:
                ...
                for _, row in df.head(5).iterrows():  # Show top 5 items
                    ...
                    output.append(f"    On Hand: {row['OnHandQty']}, Reorder: {row['ReorderPoint']}, Status: {row['StockStatus']}")
    ...
    def save_alert_log(self, alerts: Dict[str, pd.DataFrame], 
                      summary_stats: Dict[str, Any],
                      log_file: str = "logs/alerts.log") -> bool:
        """
    ...
    def send_alerts(self, df: pd.DataFrame, 
                   summary_stats: Dict[str, Any],
                   send_email: bool = True,
                   save_log: bool = True,
                   show_console: bool = True) -> Dict[str, bool]:
        """
...
def send_inventory_alerts(df: pd.DataFrame, 
                         summary_stats: Dict[str, Any],
                         config: Optional[Dict[str, Any]] = None,
                         send_email: bool = True) -> Dict[str, bool]:
    """
