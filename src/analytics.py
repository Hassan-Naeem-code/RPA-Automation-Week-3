...
class InventoryAnalytics:
    ...
    def _analyze_stock_distribution(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        distribution = {
            ...
            'reorder_analysis': {
                ...
                'average_reorder_qty': float(df[df['ReorderQty'] > 0]['ReorderQty'].mean()) if (df['ReorderQty'] > 0).any() else 0
            }
        }
    ...
    def _analyze_by_location(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for location in df['Location'].unique():
            ...
            location_stats[location] = {
                ...
                'utilization_rate': float(location_df['OnHandQty'].sum() / location_df['ReorderPoint'].sum()) if location_df['ReorderPoint'].sum() > 0 else 0
            }
    ...
    def _analyze_inventory_value(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_value_items': df_sorted.head(10)[['SKU', 'Description', 'TotalValue', 'StockStatus']].to_dict('records')
        }
    
    def _generate_insights(self, df: pd.DataFrame, stock_analysis: Dict, location_analysis: Dict, value_analysis: Dict) -> List[str]:
        """Generate intelligent insights from the analysis."""
        insights = []
        
        # Stock level insights
        critical_ratio = stock_analysis['status_breakdown'].get('Critical', 0) + stock_analysis['status_breakdown'].get('Out of Stock', 0)
        total_items = len(df)
        
        if critical_ratio / total_items > 0.05:  # More than 5% critical
            insights.append(f"⚠️ HIGH RISK: {critical_ratio} items ({critical_ratio/total_items*100:.1f}%) are in critical or out-of-stock status")
        ...
        if 'abc_analysis' in value_analysis:
            ...
            insights.append(f"💰 VALUE CONCENTRATION: {a_items['count']} items ({a_items['percentage']}%) represent {a_items['value_percentage']}% of total inventory value")
        
        # Reorder insights
        reorder_needed = stock_analysis['reorder_analysis']['items_needing_reorder']
        if reorder_needed > 0:
            insights.append(f"📦 REORDER REQUIRED: {reorder_needed} items need immediate reordering with total quantity of {stock_analysis['reorder_analysis']['total_reorder_quantity']:.0f}")
    ...
    def _generate_recommendations(self, df: pd.DataFrame, trends: Dict) -> List[Dict[str, str]]:
        ...
        if 'stock_distribution' in trends:
            ...
            critical_count = stock_dist['status_breakdown'].get('Critical', 0) + stock_dist['status_breakdown'].get('Out of Stock', 0)
    ...
    def generate_dashboard_data(self, df: pd.DataFrame, trends: Dict, predictions: Dict) -> Dict[str, Any]:
        ...
        dashboard = {
            ...
            'charts': {
                ...
                'location_bar': df.groupby('Location')['TotalValue'].sum().to_dict() if 'Location' in df.columns else {},
    ...
    def _generate_dashboard_alerts(self, df: pd.DataFrame, trends: Dict) -> List[Dict]:
        ...
        if 'TotalValue' in df.columns:
            high_value_low_stock = df[(df['TotalValue'] > df['TotalValue'].quantile(0.8)) & 
