...
class PerformanceMonitor:
    ...
    def _check_alert_thresholds(self, metric: PerformanceMetric):
        ...
        if threshold_key and threshold_key in self.alert_thresholds:
            ...
            if metric.value > threshold:
                logger.warning(f"PERFORMANCE ALERT: {metric.metric_name} = {metric.value:.2f}{metric.unit} "
    ...
    def benchmark_function(self, func: Callable, *args, **kwargs) -> BenchmarkResult:
        ...
        logger.info(f"Benchmark completed: {test_name} - {duration:.3f}s, "
    ...
    def get_performance_summary(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'metric_statistics': {},
            'recent_benchmarks': len([b for b in self.benchmarks 
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if benchmark.success:
                logger.info(f"⚡ {func.__name__} completed in {benchmark.duration_seconds:.3f}s "
                           f"({benchmark.memory_used_mb:+.2f}MB memory)")
            else:
                logger.error(f"❌ {func.__name__} failed after {benchmark.duration_seconds:.3f}s: "
