...
class MetricsCollector:
    ...
    def record_business_metrics(self, processed_data: pd.DataFrame, 
        ...
        business_metrics = {
            ...
            'low_stock_items': len(processed_data[processed_data['StockStatus'] == 'Low Stock']) if 'StockStatus' in processed_data.columns else 0,
            'critical_items': len(processed_data[processed_data['StockStatus'].isin(['Critical', 'Out of Stock'])]) if 'StockStatus' in processed_data.columns else 0,
            'items_needing_reorder': len(processed_data[processed_data['ReorderQty'] > 0]) if 'ReorderQty' in processed_data.columns else 0,
    ...
    def calculate_performance_indicators(self) -> Dict[str, Any]:
        ...
        indicators['time_savings_percent'] = round(
            (time_saved_seconds / manual_time_seconds) * 100, 2
        ) if manual_time_seconds > 0 else 0
